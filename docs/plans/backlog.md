...
**What moved on 2026-09-17, and why.** B-44 enters at the head of P2: it is the one
acceptance check a delivered item left unperformed, it is ungated and small, and the
tables' keyboard/header work it rests on is already browser-verified — so it is the only
item the next implementation round can start without an operator decision. B-45 enters
P3 as test hygiene (the stored-data browser specs reach a live UniProt resolve); it adds
no user capability. B-11's remaining half moves P2/NEXT → P3/LATER: the refusal UX is
delivered, the gate requires one provider to be real first, and a second endpoint
measures compatibility rather than adding anything a user can do (`AGENTS.md`
...
**What moved on 2026-09-17 after the keyboard-model round (round 3).** **B-46 is
delivered** and leaves the table: the tables are one tab stop each, the arrow keys move
the current row (and the reader now hears it, because the row carries a name), and the
two keyboard-model defects B-44's pass measured are gone by measurement. The cells are
still not focus targets, so cell-level arrow navigation is not delivered and is stated
as a limit rather than a claim; nothing new enters the table for it, because no
reviewed task asks for a two-dimensional grid yet (`AGENTS.md` §37). The P2 head is now
**B-21** (the operator's claims-source decision), then **B-09** (scientific review).

**What moved on 2026-09-17 after the navigation round (round 4).** **B-43 is delivered**
and leaves the table: one navigation step is one history entry, verified before (two
entries, two Backs) and after (one entry, one Back) with B-39's per-entry filter state
pinned in the same run. With it the register holds **no ungated engineering item**:
every remaining row is either the operator's (B-31, B-21, B-09, B-11, B-20, B-16) or
gated on a measured need or a decision this checkout cannot produce on its own (B-40's
reviewed comparison task, B-05's dataset and ADR, B-07's bounded design, B-12's scaling
need, B-22's source decision, B-27's user ask, B-28's repeated-batch volume, B-41's
pilot friction, B-45's no-network runner, B-47's model decision). The next implementation
...
| B-17 | 2026-09-16 | One runner, one smoke of the MVP loop: `@playwright/test` (dev dep, Apache-2.0; §23 decision + alternatives recorded in `THIRD_PARTY_NOTICES.md` §2 in the same change), `apps/web/playwright.config.ts` (baseURL `SPAGO_BASE_URL`, default `http://127.0.0.1:8000`), `apps/web/e2e/smoke.spec.ts` (search `DEMO-PATENT-A` → family table → row opens the **evidence inspector** → `Structure ▾` aspirin substructure → table restates `(structure search)` ≥ 1 row → `Export ▾` yields a `spago-export.csv` download), `npm run test:e2e`, artifacts git-ignored. Passed in 1.6 s against the rebuilt stack — which also applied migration 0020 to the stack database; `npm run build` unchanged. The first run corrected the spec's own assumption: a row click opens the evidence inspector, not a structure drawer — the smoke follows the real UI, no app code changed. Limits stated in the plan (`docs/archive/2026-09-16-e2e-smoke.md`): the smoke is not wired into CI (CI has no seeded stack), and it does not duplicate the Python suite's API coverage. |
| B-33 | 2026-09-16 | Target screen/export parity: `ExportRequest.evidence_class` (validated five-class vocabulary, 422 on a family scope that sends it) + the same `current_measurements` existence clause the screen uses in `collect_candidate_export_rows` (results scope only — a selection names its own rows), the frontend passing the live `referenceThresholdNanomolar` / `evidenceClassFilter` through `ExportMenu` and `api.exportFile`, and a truthful export hint. Browser-reproduced first on the stored IL6 investigation (menu claimed 141, file had 157 rows, every `reference_threshold_nM` read 10000 against a 1 nM override) and browser-verified after (141 = 141, threshold 1 nM; selection 1 row, threshold 1 nM); `tests/test_b33_export_parity.py` (8 cases, per-class export == screen set) + 104-test regression green. Plan: `docs/archive/2026-09-16-export-policy-parity.md`. Found and recorded en route: **B-42**. |
| B-34 | 2026-09-16 | Distinguishable build identity: `SPAGO_BUILD_ID` env injected at packaging (`docker/app/Dockerfile` build arg baked into the image, compose `${SPAGO_BUILD_ID:-unknown}`, `scripts/build_app.sh` computing `git describe --always --dirty`), `/healthz` carries `build_id` + `build_source` with explicit `unknown` (never guessed from `api_version`), and `scripts/build_identity.py --expected <id> [--require]` compares expected vs served and fails on mismatch/unknown. Verified end-to-end on the rebuilt stack: `/healthz` served `12aa498-dirty`, recorder exit 0. The round's own review found the build script exporting an empty variable (never the computed id) — fixed and caught by exactly the served-artifact check the item exists for; the lesson is in the plan. `tests/test_b34_build_identity.py` (6 cases). Plan: `docs/archive/2026-09-16-build-identity.md`. |
| B-42 | 2026-09-16 | Two CSS rules (`apps/web/src/styles.css`): `.table-panel` keeps a 320 px minimum instead of `min-height: 0` — flex shrink previously collapsed it (and the virtual body) to ~32 px under tall sibling blocks — and `.table-area` scrolls when its content overflows, so short viewports page through the header content instead of hiding the table. Verified on the rebuilt stack (`3dfd6fc-dirty`) at **1280×720** (0 → 9 rendered rows, `.table-scroll` 237 px, row selection and the Selected (1) export control work) and at **1600×1000** (10 rows, no regression); the family-view smoke passes unchanged. Layout-only change: no component, state or API touched. |
| B-35 | 2026-09-16 | A `full-stack` job in `.github/workflows/checks.yml`: builds and starts the **shipped** db image through docker compose, runs the full backend suite with `SPAGO_REQUIRE_TEST_DATABASE=1` (unreachable database/missing cartridge fails instead of skipping — and `test_migration_0006_upgrade.py` now respects that gate like conftest and the 0008 helper), builds the app image with `SPAGO_BUILD_ID` from `git describe`, verifies the served identity via `scripts/build_identity.py --require`, and runs the B-17 smoke against the seeded stack. **Verified by executing the hosted run 35113553962** (commit `d9f441a`): db healthy, **793 passed** in 72 s, `OK: served build_id matches d9f441a`, smoke 1 passed in 2.7 s; jobs `checks` 1m08s + `full-stack` 3m09s. Honest limits: fixture CI on synthetic data, no paid calls — not hosted acceptance or live-source/model validation; the Node-20 actions annotation remains cosmetic. |
| B-36 | 2026-09-16 | Every saved scope of a mixed project reopens: `openProjectTarget` (the extracted sibling of `openProjectFamily`, with navigation cancellation so an older response cannot overwrite a newer switch), the `openedProjectScopes` memo (families **and** targets with saved-item counts; all-missing scopes stay listed), and one "Saved scope" switcher in both banners replacing the families-only select; the target banner gained the drift labels the family banner already had. Browser-verified on `d9f441a-dirty` at 1600×1000: a project holding DEMO-FAMILY-1 + IL6 + TSLP reopens its family scope with a three-option switcher, navigates to each target and back, banner labelling the open scope throughout; project shape confirmed in the database, test projects removed. Not browser-covered: owner isolation (auth disabled locally; API-tested) and a genuinely missing scope. Plan: `docs/archive/2026-09-16-mixed-project-scopes.md`. |
...
| B-46 | 2026-09-17 | The tables are one tab stop each, and the reader can hear where it is. Roving `tabIndex` (the current row is the only one at `0`; the current row's nested controls follow it), `ArrowDown`/`ArrowUp`/`Home`/`End` moving the current row with the virtualizer scrolling it into view and focus following after it renders, and an `aria-label` on every row carrying its identity and the content its cells hold. Measured with `npm run check:table-keyboard`: Tab stops inside the compound table **30 → 3** and inside the candidate table **to 1**, arrow moves landing on rendered rows, Home/End at the ends, Enter still opening the Evidence inspector and Space on a checkbox still selecting without inspecting; 60 rapid `ArrowDown` presses advance 60 rows with 16 rendered and no press lost (the first attempt advanced 30 — reading the position back from the focused element stalls a held key, so it is tracked in a ref). Orca 42.0 reads the rows in the same run: `Compound 2 of 10: BSYNRYMUTXBXSQ-UHFFFAOYSA-N, C9H8O4, 2 measurements, 2 patent labels`, `Candidate 2 of 157: WEBQKRLKWNIYKK-UHFFFAOYSA-N, weak IC50 13810 nM, measured binding, no patent mapping`. Verified on `0d1d983-dirty`: `npm run test:e2e` 8 passed, `scripts/run_checks.sh` green. Not delivered: cell-level arrow navigation (the row's name carries the content instead) and any claim about Orca's own Ctrl+Alt+arrow commands. Record: `docs/archive/2026-09-17-table-keyboard-model.md`, measurements `benchmarks/table-keyboard-2026-09-17.{md,txt}`. |
...
**Delivered 2026-09-16** (`docs/archive/2026-09-16-source-declared-linkage.md`). The three
...
**Delivered 2026-09-16** (`docs/archive/2026-09-16-tolerant-publication-lookup.md`). The
...
**Delivered 2026-09-16** (`docs/archive/2026-09-16-import-refresh-completeness.md`; see
...
**Delivered 2026-09-16** (`docs/archive/2026-09-16-analysis-history.md`). The shipped
...
- **Delivered 2026-09-16 (ungated half):** the refusal detail names the class and the
  remaining budget, with an explicit Retry for the billed call (see §1).
- **Class LATER · P3 · M (reclassified 2026-09-17: the refusal half is delivered, and a
  second provider adds no capability the acceptance gate asks for — it may still be
  chosen by the operator, but it no longer displaces startable work).**
...
**Delivered 2026-09-16** (`docs/archive/2026-09-16-e2e-smoke.md`; see the delivered
...
- **Delivered 2026-09-16:** the keyboard/header half, browser-verified (see §1). The
  assistive-technology announcement pass was **not performed** and is now **B-44** —
  this item is closed, that one keeps the acceptance open.
- **Class NEXT · P2 → keyboard half delivered; the reader pass is B-44.** Promoted from
  LATER because the affected controls already ship.
...
**Delivered 2026-09-16** (`docs/archive/2026-09-16-bindingdb-snapshot.md`; record
...
**Delivered 2026-09-16** (`docs/archive/2026-09-16-patent-source-compounds.md`). The problem
...
**Delivered 2026-09-16** (`docs/archive/2026-09-16-supplement-bundle-import.md`). The problem
...
`docs/archive/2026-09-16-patent-coverage-audit.md`). Kept here as the item's record: what was
...
**Delivered 2026-09-16** (see the delivered table in §1) — two CSS rules, verified
at 1280×720 and 1600×1000 on the rebuilt stack, family smoke unchanged.
...
**Delivered 2026-09-17** (see §1's delivered table; the reproduction, the change and the
after-measurement are in `docs/archive/2026-09-17-history-step.md`). The item statement
below is kept as the record of what was found.
...
- **Evidence.** B-19 moved `role="table"`, the header row and `columnheader` cells into
  ...
  usable focus during virtual scrolling — was never performed. `AGENTS.md` §27 now
  requires the pass to be recorded separately, so it cannot be closed by the DOM work.
- **User gain / scope.** A screen-reader user can tell what a cell means and where they
  are in a virtualized result table; scope is the two existing tables and the existing
  layout, not a redesign.
- **Acceptance.** Announcement pass performed with a named reader/platform and recorded
...
**Delivered 2026-09-17** (see §1's delivered table; the measurements are in
`benchmarks/table-keyboard-2026-09-17.md` and the plan, with its outcome and the two
things it did not anticipate, is `docs/archive/2026-09-17-table-keyboard-model.md`). The
item statement below is kept as the record of what was asked.
...
| 2026-09-16 | **B-42 delivered** in the same round (`apps/web/src/styles.css`: `.table-panel` min-height 320 px replacing `min-height: 0`; `.table-area` overflow-y auto). Browser-verified at 1280×720 (0 → 9 rendered rows, selection and export control work) and 1600×1000 (no regression); the family-view smoke passes. **B-42 leaves the table; the P1 head is now B-32's machine-preparable cohort work (its independent-review half stays with the operator), followed by B-35** (CI full suite — now unblocked to also stamp build identity into CI runs via B-34's recorder). |
