...
日期：2026-09-14；更新：2026-09-15。状态：设计参考，已有部分实现；草稿图不是运行截图。当前核对基线：`f3da90c`，本轮仅做文档、定向代码/本地契约检查，未重跑浏览器验收。

## 依据与 Q&A

初稿编写时仓库只有文档。当前已存在 React 前端、FastAPI 服务、迁移、合成 Parquet 夹具及 M0–M5 实施记录；不能继续把初稿阶段描述当作现状。实施与验证缺口见 [本轮复查及下一轮计划](../archive/2026-09-14-ui-review-next-round.md)（已归档）。历史测试结果保留在原实施记录中，本轮未重新验证。

| 问题 | 当前设计决定 / 待确认项 |
| --- | --- |
| 首期服务哪个任务？ | 输入专利公开号，查看同族和结构列表，核对来源，保存所选对象。 |
| 是否直接实现完整愿景？ | 按 README 的 M0/M1 起步；PROMPT 的完整 MVP 路径作为后续里程碑组合。 |
| 默认同时显示 Evidence 和 AI？ | 否。证据按需展开；已实现的 AI 复用右侧检查区，通过标签切换，引用可切回证据。 |
| 首期是否有全文/PDF 定位？ | 取决于实际数据覆盖。只有数据库关联时展示来源记录；缺页码或原文时明确说明，不伪造定位。 |
| AI 标签模式选择？ | 当前已实现：AI 视图显示同族范围、模式选择（离线摘要/已配置时的 LLM 摘要）、模型与脱敏目标、缓存复用与可点击引用；证据来源标签只在 Evidence 视图。真实模型验收未完成。 |
...
## LLM 接口补充（2026-09-15，已实现并有待纠偏项）

详细协议、实现记录与 LLM-05–07 纠偏见 [LLM 接口计划](../archive/2026-09-14-llm-interface.md)（已归档）。现有 AI 标签与同族摘要 API 已接入可选 adapter；后端配置 endpoint/key/model，界面显示脱敏状态、摘要模式、一次显式生成动作和可点击引用。首期不增加聊天侧栏或可写密钥设置页。未配置可用离线摘要，配置齐备不等于已验证连接；真实模型输出标为 LLM inferred，失败不冒充离线成功。LLM-05–07 已修复，并在隔离栈的 1440×900 与 760×800 用本地 mock 端点验收；真实模型 smoke 仍是缺口，原草稿图不是这些能力的运行证据。

当前修复沿用上述布局：普通列表通过服务端 offset 分页继续加载到 500 条之后（每页 100 条、末页停用 Load more）；结构分页失败保留已加载行并显示可重试错误。翻页不混淆查看对象、批量选择、证据与保存/导出范围；过期请求和错误作废，查看行与结构抽屉在结构结果范围内解析。实现与浏览器验收见当前修复计划，不增加永久面板或重复分页操作。

## 生成与检查记录

使用内置 image_gen，未使用 CLI/API fallback。完整生成提示词和修订提示词见 [归档记录](../archive/2026-09-14-imagegen-prompts.md)。
