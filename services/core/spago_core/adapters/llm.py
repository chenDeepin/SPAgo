    ...
    def _system_prompt(self) -> str:
        ...
        if self.scope == "target":
            return (
                ...
                "The input also carries a `reference` fact: a deterministic count of how "
                "many in-scope compounds are at or below an explicit threshold, under the "
                "policy printed in that fact. Cite `reference:<id>` for any statement "
                "about potency or about the set being usable or not. Repeat its numbers "
                "and threshold exactly; never recompute them, never apply a different "
                "threshold, and never turn the count into a claim about the literature "
                "(it says what the stored records support, not what exists).\n"
                "Each measurement and candidate carries its own `activity_class` from "
                "that same policy: it classifies ONE report, so do not present it as an "
                "independent finding about the compound.\n"
                ...
                "proof that a compound is unclaimed. A `source_declared_patents` value is "
                "what a database declared, not proof that the compound occurs in that "
