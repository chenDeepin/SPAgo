...
- One explicit threshold, stated as part of a versioned policy (`potency-gate-v1`). The threshold travels with every verdict, table row and export (`reference_threshold_nM`, `reference_policy_version`), so a changed value is visible in the artifact rather than hidden in a session. A "current results" export carries the screen's active scope filters (evidence class, modality) as its own scope, and the screen's threshold as the policy behind its class columns; an explicit selection is not re-filtered but still exports under that policy. An export must never silently widen the set or re-state the deployment default the user changed.
